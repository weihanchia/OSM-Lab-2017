...
def EV_F(X, k_init, n_agents, zstate):
    ...
    for i in range(len(theta)):
        ...
        VT_sum += beta*V_INFINITY(theta[i], knext)*pi[zstate, i]
...
def EV_F_ITER(X, k_init, n_agents, grid, zstate):
    ...
    for i in range(len(theta)):
        VT_sum=utility(cons, lab)
        VT_sum += beta*grid[str(i)].evaluate(knext)*(pi[zstate, i])
...
def EV_GRAD_F(X, k_init, n_agents, zstate):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F(xAdj, k_init, n_agents, zstate)

            xAdj[ixN]=X[ixN] - h
            fx1=EV_F(xAdj, k_init, n_agents, zstate)
...
def EV_GRAD_F_ITER(X, k_init, n_agents, grid, zstate):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F_ITER(xAdj, k_init, n_agents, grid, zstate)

            xAdj[ixN]=X[ixN] - h
            fx1=EV_F_ITER(xAdj, k_init, n_agents, grid, zstate)
...
def EV_G(X, k_init, n_agents, zstate):
    N=len(X)
    ...
    f_prod=output_f(theta[zstate], k_init, lab)
...
def EV_G_ITER(X, k_init, n_agents, zstate):
    N=len(X)
    M=3*n_agents+1  # number of constraints
    G=np.empty(M, float)
...
def EV_JAC_G(X, flag, k_init, n_agents, zstate):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G(X, k_init, n_agents, zstate)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G(xAdj, k_init, n_agents, zstate)
...
def EV_JAC_G_ITER(X, flag, k_init, n_agents, zstate):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G_ITER(X, k_init, n_agents, zstate)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G_ITER(xAdj, k_init, n_agents, zstate)
                A[ixN + ixM*N]=(gx2[ixM] - gx1[ixM])/h
        return A
