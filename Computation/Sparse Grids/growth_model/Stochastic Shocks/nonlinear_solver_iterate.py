...
def iterate(k_init, n_agents, grid, zstate):
    ...
    def eval_f(X):
        return EV_F_ITER(X, k_init, n_agents, grid, zstate)

    def eval_grad_f(x):
        return EV_GRAD_F_ITER(X, k_init, n_agents, grid, zstate)

    def eval_g(x):
        return EV_G_ITER(X, k_init, n_agents, zstate)

    def eval_jac_g(x, flag):
        return EV_JAC_G_ITER(X, flag, k_init, n_agents, zstate)
